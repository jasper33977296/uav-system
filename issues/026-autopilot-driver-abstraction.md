...
計畫見 [`doc/autopilot-driver-architecture.md`](../doc/autopilot-driver-architecture.md) §9。

**開工前先解決一個會讓整個計畫走偏的問題**：待決點 2 原本寫著「地面站不再持有
驅動」，**照字面做會壞掉**——「這台機有沒有代理」是**執行期的事實，不是編譯期
的**。SITL 沒接代理就整個開發流程斷掉；**第三方的機永遠不會有代理**。
所以驅動不從地面站刪除。

而處置不是「有代理就用代理的」——協定 §4.2 早就定了一條更好的規則，B4 沿用它：

* 地面站**認得**那個廠牌 → 兩邊各自算，**不一致就報警**（不是拿來選一邊的）
* 地面站**不認得**（新廠牌）→ 沒有意見，用代理的值並標明來源。
  **這才是搬家真正的收穫**：加一款自駕儀時，只要那台機上有代理，
  地面站一行都不用改
* **沒有代理** → 照舊由地面站自己解讀

### 已完成：B4-a（協定）＋ B4-c 的地面站半邊

* 協定 §4.2 補上正規化欄位的契約（`mode_verb`／`mode_name`／`ready`／
  `not_ready_reasons`／`prearm_ok`／`ekf_ok`），並訂死一條：
  **缺欄位＝代理沒說，不是值是 false**——舊版代理不送這些欄位，讀成
  `ready: false` 會讓所有還沒升級的機都變成不可飛（與 039 複裁 A 的
  `rc_link` 同一個教訓）。
* `hello` 補 `driver`（待決點 6）：代理自報驅動名稱、版本與 `limits`。
  「版本不合就降級唯讀」那類守則**本批不做**——**沒有人看得到的漂移，
  訂再多守則也管不到**，先讓差異可見。
* **執行期護欄已上線**：`agent_link.crosscheck()` ＋ backend 的
  `_crosscheck_normalized()`。兩邊看的是同一份遙測，算出不同結果代表
  **有一邊的驅動是錯的**，那是要修的 bug 不是要調解的分歧。
  持續 3 秒以上才報（模式切換的當下兩邊必然短暫不一致，立刻報就是 002
  那種抖動狂噴）。驗證：`scripts/test-driver-crosscheck.py`，10 項全過，
  含「舊版代理不會被誤報」與「不認得的廠牌不比」兩條反向驗證。

> **為什麼護欄要在執行期而不是只在測試裡**：兩份實作並存的這段期間，
> 是唯一能做這個比對的窗口。而它讓「搬家有沒有搬對」變成一個持續被回答的
> 問題，不是只有跑測試那一刻才問得到。

### 待做

* **B4-b（機端）**：把驅動移植到代理，由它解讀後送正規化值。交給機端 session。
* **B4-d**：一致性測試套改成兩邊都跑，同一台 SITL 比對逐鍵相同。
* **B4-e**：待決點 6 的機隊漂移呈現（地面站看得見版本差異）。
