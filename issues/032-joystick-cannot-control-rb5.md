...
- 狀態：**closed（2026-08-24，隨功能移除，非修復）**
...
## 結案（2026-08-24）：隨功能移除，**不是修好了**

虛擬搖桿整塊已移除，見 [035](035-remove-manual-control.md)——系統範圍收斂為
「航路管理＋飛行安全」，連續操縱交給實體遙控器。本案追查的根因**至今仍未確認**。

**檔案保留的理由**：裡面對 `COM_RC_IN_MODE`、轉發過濾、GCS 身分、流逾時
這四個候選的排查與 spec 對照，對日後任何「地面站送出去了、機端卻沒反應」
的問題仍然適用——那類故障的共同特徵正是本案的核心：**靜默丟棄、沒有任何
錯誤訊息**。
