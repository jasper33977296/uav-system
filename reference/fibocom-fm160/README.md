...
| 8 | **physicalcellId** | 85 | **十六** | PCI = 133（見下方修正）|
...
### `physicalcellId` 是十六進位（2026-09-07 修正）

上表原本把第 8 欄記成十進位——**那是從一筆「85」推出來的，而 85 在兩種進位下
都合法，推不出結論**。本場域後來的實測值是 `8D`：它有字母，十進位讀不了。
所以這一欄與它前後的 tac／cellid／narfcn 一樣是十六進位（`8D` → PCI 141，
T-Mobile 範例的 `290` → 656，兩者都在 NR PCI 的合法範圍 0–1007 內）。

解析實作與自我檢查在 `apps/backend/app/modem_raw.py`（`python3 -m app.modem_raw`）。
...
* 範例的 narfcn `7EFAE` = 520110，落在 **n41**（499200–537999）值域，
  而它的 band 欄是 **5041**。
  （原本這裡寫 519086——**算錯了**，2026-09-07 實作解析器時被自我檢查抓到。
  結論不受影響：520110 一樣在 n41 值域內。）
