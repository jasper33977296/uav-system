#!/usr/bin/env python3
"""外部控制的影像欄位（issues/022；2026-09-23 使用者裁定走 MJPEG over HTTP）。
...
check("ready 給得出網址", r["mjpeg"] == f"http://10.141.2.21:38000/api/drones/{DID}/camera/stream.mjpg", r)
...
check("offline 不給網址", r["mjpeg"] is None, r)
...
check("自訂埠會被帶進網址",
      V.mjpeg_url(DID, "h", 9999) == f"http://h:9999/api/drones/{DID}/camera/stream.mjpg")
...
for d in drones:
    ...
    if v.get("state") == "ready":
        check(f"{tag}：ready 有 mjpeg 與 snapshot",
              bool(v.get("mjpeg")) and bool(v.get("snapshot")), v)
        check(f"{tag}：主機名跟著請求的 Host",
              (v.get("mjpeg") or "").startswith(f"http://{HOST.split(':')[0]}:"), v)
        # **不保證拉得到**：這裡只驗契約有把話說出來，不驗真的拉得到畫面
        check(f"{tag}：有說明 ready 不等於一定有畫面", bool(v.get("note")), v)
    else:
        check(f"{tag}：非 ready 就不給網址", v.get("mjpeg") is None, v)
