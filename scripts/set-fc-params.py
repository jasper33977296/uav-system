...
PLAN = {
    ...
    "BATT_CAPACITY": (4200, "實際電池是 4S **4200 mAh**（2026-09-07 使用者確認，"
                            "充電器滿電 16.8 V＝4×4.2 也對得上），而飛控上是 3300。"
                            "**剩餘電量就是拿這個算的**——`(容量−已耗)/容量`，"
                            "少報 900 mAh 會讓飛控在還有約 21% 電時就報 0%。"
                            "方向雖然偏保守，但那是**用錯的數字換來的保守**，"
                            "而失聯接管的兩個判準之一正是這個百分比"),
    ...
    # ── 圍欄：拿掉多邊形那一位（2026-09-21 使用者裁定）────────────
    "FENCE_TYPE": (1, "5＝高度上限(1)＋多邊形(4) → 1＝只留高度上限。"
                      "飛控裡存著一個六點多邊形（FENCE_TOTAL=6），而機子站在它"
                      "外面，`PreArm: Vehicle breaching Polygon fence` 擋住解鎖。"
                      "**那個多邊形已經沒有人在管**：d4b785d 拿掉了上傳航線時對機上"
                      "FENCE_* 的配置之後，我們既不寫也不顯示它，所以它是那次改動"
                      "之前留下來的舊資料——**站在一個看不見的圍欄外面**。"
                      "頂點本身不是參數（MAV_MISSION_TYPE_FENCE 的 mission items），"
                      "這支工具刪不掉，所以是讓飛控不再檢查它，不是清掉它。"
                      "**後果**：有人把這一位改回去，舊多邊形就會再活過來"),
}
...
NOT_TOUCHED = {
    ...
    "FENCE_ALT_MAX": "**現值 6 公尺，而 FENCE_ENABLE 仍是 1、FENCE_TYPE 改成 1 之後"
                     "高度上限正是唯一還生效的那一項**——爬過 6 m 就觸發"
                     "FENCE_ACTION=1（RTL／降落）。2026-09-21 使用者裁定這次不動，"
                     "所以它不是漏掉的。**要實飛到 6 m 以上之前必須先處理這一格**，"
                     "而合適的值由場地決定，機器決定不了",
    "FENCE_ENABLE": "維持 1。只拿掉多邊形那一位（見 PLAN 的 FENCE_TYPE），"
                    "不是把整個圍欄關掉——高度上限要留著",
}
