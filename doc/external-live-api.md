...
```json
{"detail": {"code": "mission_busy",
            "msg": "這台機（sysid 1）已經在進行中的任務「0914 巡檢」（8f0c…）裡，一台機同一時間只能在一個任務",
            "how_to": ["等那個任務結束再起飛", "或改用 mission_id 8f0c… 起飛，把這一趟掛進那個任務"]}}
```
...
（`reason: "never_started"`，附 `msg`）並關閉。**收到** `/api/start` 那一刻就進 `starting`，
...
{"v": 1, "type": "error", "mission_id": "8f0c…", "ts": "…",
 ...
 "how_to": ["要看這次飛行的完整資料：GET :38000/api/sessions/{session_id}/export"]}
```
