...
## 部署拓撲（2026-08-05 定案：本專案部署於地面站）

**SSH 只用於一次性安裝**（裝機上程式、設 systemd 自啟），運行期間沒有任何
人為連線——無人機開機、連上 5G，資料經兩個協定自動流向地面站：

```
  無人機（RB5：PX4 + ROS 2 + RM500Q）
  │ ① MAVLink/UDP：機上 mavlink-router →
  │     地面站:14550（QGC 控制）＋ 地面站:14540（本系統，只讀遙測）
  │ ② HTTP：機上 ROS node →
  │     POST /api/link-metrics/live（即時）＋ /batch（緩衝補傳，唯一入庫路徑）
  ▼
  地面站（Ubuntu，docker compose 跑本專案；LINK_SOURCE=modem）
```

三條流**全部由機上主動外連**：MAVLink 機上先發、QGC 沿原路回覆，HTTP 純外連
——穿 NAT 天然可行。實驗室私有 5G 直連即可；若走公網電信，加一層
WireGuard（機上撥回地面站）一次解決位址問題。

明確不採用：SSH 進機上取資料（互動通道，無重連/緩衝/補傳語意，
是維護工具不是資料鏈路）。
...
## ⚠ 2026-09-02：記錄通道**從來沒有被真機走過**

設計是兩條通道，而機上只實作了一條。`modem.py` 只往 `/live` 送——而那個端點的
第一行就寫著「**不寫資料庫**」。`/batch`（「唯一的入庫路徑」）的唯一呼叫者是
`scripts/fake-onboard-node.py`，**一支假的測試節點**。

後果：**這套系統的研究核心資料，從真機上一筆都沒有存過。**

| | |
|---|---|
| `link_metrics` 最新一筆 | **2026-08-13**（模擬／RB5 時代的殘留）|
| 09-02 當天五趟真機飛行的訊號筆數 | **全部 0** |
| 同期機上 `posted` 計數 | 一路增加，看起來完全正常 |

> **兩邊都「正常」**：畫面上有訊號、端點回 204、機端的 `posted` 一直加——
> 而資料庫裡什麼都沒有。**一條被假節點測過、被真機從未走過的路。**

修法不是改設計（設計是對的），是**把機上那半接上**：`modem.py` 現在每筆樣本
帶 `seq` 進環形緩衝，每 10 秒送一批到 `/batch`，**只刪掉地面站回報收下的那幾筆**
（`accepted_seq` 刻意包含它判定落在架次外而丟棄的，不然機上會永遠重送）。
驗收 `scripts/test-link-metrics-recording.py`。

**留下的通則**：一條路徑如果只有假的東西走過，它就等於沒有被測過。
`fake-onboard-node.py` 讓這條路「看起來有測試」，而那正是它躺了三週沒有人
發現的原因。

## 什麼時候記：離地才記（2026-09-07 使用者裁定）

> 「只有當飛離地面才需要紀錄，當無人機停在地面上時就停止紀錄。」

停機坪的訊號不是航路上的訊號。而原本的閘門在地面站那側、判準是「架次內」
（＝解鎖到上鎖）——**解鎖之後起飛之前那段仍在地上**，比要的寬。

**閘門放在機上，判準是「離地」**：

| 情況 | 記不記 |
|---|---|
| `landed_state` ＝ `on_ground` | 不記 |
| `in_air`／`takeoff`／`landing` | 記 |
| 韌體不報 `landed_state`，但高度 < 1 m 且地速 < 0.5 m/s | 不記 |
| **什麼都不知道** | **記** |

最後一列是刻意的：**飛行中的量測補不回來，多記幾筆地面資料刪得掉。** 這與
代理狀態機那條「寧可判成飛行中」是同一個方向，而且用的是**同一個函式**
（`Agent._landed()`）——「在不在地上」在這支程式裡只有一份定義。

即時通道不受影響：起飛前要看得到訊號好不好，那是畫面不是紀錄。

跳過的筆數進 `stats()` 的 `on_ground`。**丟掉多少要看得見**——「送出去了」
與「記下來了」之間的無聲落差，正是上面那件事躺了三週的原因。

**地面站不要再加第二道離地判斷。** 它手上只有樣本裡的 `alt_rel`，而
`alt_rel` 是相對起飛點的高度——低空飛行完全合法，拿它當閘門會把真的航路
資料丟掉。機上有 `landed_state`，判斷就留在機上。

## 取樣率：設定多少就要真的是多少（2026-09-07）

`--modem-interval 1.0` 實際量到 **2.61 秒一筆**（0.38 Hz），而 §3.6 寫的是
1 Hz。**兩個各自獨立的錯疊在一起：**

| 錯 | 內容 |
|---|---|
| 睡錯地方 | `time.sleep(interval)` 睡在工作**之後** → 週期＝工作＋interval |
| 等太久 | `_at()` 無條件睡滿 `wait` 再讀，不看數據機多快回 |

實測 FM160：三個 AT 指令合計**回覆只要 49 毫秒**，程式卻等 1.6 秒——
0.5／0.7／0.4 那三個數字是「猜一個夠久的」，猜保守了 30 倍。

修法：`_at()` 讀到 `OK`／`ERROR` 為止（`wait` 變成逾時上限，**最壞情況不變**），
迴圈改睡「還差多少」。修後真數據機單次取樣 **50 毫秒**。

**第一個錯在工作很快時完全看不出來**（週期≈interval），那正是它活這麼久的
原因；而唯一看得出來的地方是 `link_metrics` 的時間戳差，那張表當時沒有真
資料。**能揭露問題的資料，正好就是沒被記錄的那份。** 現在 `stats()` 直接
報 `hz`，不必再靠事後推。

## 蒐集密、傳送疏（2026-09-07 使用者裁定）

> 「訊號資訊蒐集頻率可以高，但傳送可以每秒傳送一次。」

兩條通道的需求本來就不同，綁在一起等於**讓畫面的需求決定研究資料的解析度**：

| 通道 | 頻率 | 為什麼 |
|---|---|---|
| 取樣 → 記錄緩衝 | `--modem-interval`（預設 1.0s） | 研究要密度；每筆都帶當下位置 |
| 即時 `/live` | `--modem-live-interval`（預設 1.0s） | 畫面只要知道「現在好不好」 |
| 記錄 `/batch` | 每 10s 一批 | 批次、可重送、冪等 |

**取樣可以比傳送密**：飛行中位置是連續變的，所以即使數據機的讀數還沒更新
（靜止實測約 1–4 秒才變一次），密一點的樣本仍然提高「訊號 vs 位置」的空間
解析度。5 m/s 飛行下，1 Hz＝5 公尺一點。
