...
## 機上錄製：開機就錄（2026-09-07 使用者裁定）

> 「錄製資料是 tlog，那機上只要開機就要持續錄製，並在無人機在地面上時持續
> 傳送到地面站，直到開始任務時才錄製但不傳送，避免影響流量，直到回到待機
> 狀態才繼續傳送。」

**傳送那半本來就是這樣**（`uploader` 的四道門：沒 armed／落地靜置／鏈路通／
有 board_uid）。改的是**錄製**那半：原本 `armed_only=True`，於是地面待機那段
機上完全沒有紀錄——而兩層收集的價值來自兩份的差，機上在地面時是空的，
等於兩層只有一層。

### 為什麼一定要配時間輪替

檔案是在「解鎖→上鎖」那個邊界關的。開機就錄之後，**一台從頭到尾沒解鎖的機
永遠不會有那個邊界**，檔案要寫滿 `max_mb` 才關——實測 3.8 KB/s，512 MB 要
**37 小時**。而沒關好的檔案不會被自動回傳（內容還會變，sha256 與續傳都無從
談起）。所以「開機就錄」如果不配輪替，會**安靜地連帶關掉自動回傳**。

`--record-rotate-min`（預設 30）。**只在未解鎖時輪替**：飛行中換檔會把一趟
飛行切成兩份，而「一趟飛行一個檔案」是回放與兩層比對的前提。

### 代價要說出來

30 分鐘一份、3.8 KB/s → 約 6.8 MB／份、**48 份／天、328 MB／天**。
以 256 KB/s 的上傳限速算，約占 5G 上行 **21 分鐘／天**（都在地面時段）。
`--record-keep` 預設 20 份，在連續錄製下只剩**約 10 小時**的機上留存——
但檔案在地面會很快傳回地面站，所以滾動刪掉的多半是已經傳完的。
真正的損失只發生在「鏈路長時間不通 ＋ 錄製持續累積」，而那種情況滾動本來
就優先於保住紀錄（卡滿了會讓整台機出問題）。
