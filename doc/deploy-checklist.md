...
> **2026-08-31 新增元件 `uav-heartbeat`**（issues/033 §4.2.1）：GCS 心跳已從
> command 服務搬出成獨立容器。起服務後要確認它在跑——
> `docker compose ps uav-heartbeat` 應為 `Up`，log 應出現「心跳中：sysid N→…」。
...
- [ ] **機上錄製的空間**：`ssh pi@<機> df -h /home` 與 `du -sh ~/uav-record`。
      代理會滾動（**每次開檔時**留最新 `RECORD_KEEP` 份），但**卡滿了會讓整台機
      出問題**——包括代理自己（寫不了 log、補傳緩衝也存不下）。
      > **一趟飛行約 4 MB**（UART 5.6 KB/s × 十分鐘），一趟一個檔案——
      > 所以 512 MB 的換檔門檻在真機上幾乎不會觸發，**空間是由「保留幾份」
      > 決定的，不是由換檔大小**。
- [ ] **機上錄製會自己傳回來**（agent v0.6.0 起，issues/014）：
      `curl :38000/api/onboard-captures` 應該看得到這台機的檔案、`complete: true`。
      **不必再 `scp`**。回傳**只在地面進行**（armed 時完全不傳、傳到一半起飛
      立刻停），落地靜置 20 秒後開始、限速 256 KB/s。
      > 沒看到東西時先看機端 `journalctl -u uav-agent | grep 回傳`：它會說出
      > **為什麼**沒傳（armed／落地靜置中／地面站失聯／還沒拿到 board_uid），
      > 而不是安靜地什麼都不做。
      > 要退回人工取回：`RECORD_UPLOAD=0` ＋ `scp pi@<機>:~/uav-record/*.tlog ./`
- [ ] **機上還有幾份沒傳回來**：前端 **`/captures` 頁**（頂欄第五個 icon）
      或代理狀態列的 `upload.pending`。**一直不降代表守門一直沒開**
      （還在飛？失聯？），不是「沒有東西要傳」——所以那一頁一律寫得出原因。
      同一頁也看得到**這一趟地面站瞎掉的那幾段機上補到了沒有**
