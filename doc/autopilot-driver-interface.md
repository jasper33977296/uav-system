...
**現況待修的實例**：監督式起飛（`TakeoffIn.alt`）仍是全域預設 `10.0` 不分廠牌，
前端那一格也寫死 min 3／max 100／預設 10。B2 落地後兩邊都改由 `limits()` 供給。

> 2026-09-07 更正：**`mission_fly` 這一條已經不是全域常數了**——它改取任務自己的
> `NAV_TAKEOFF` 高度（讀不到才退回 1 m 保底）。那不是 `limits()`，是「這個值本來
> 就該由任務決定、不該由我方挑一個數字」；`limits()` 要解的仍是監督式起飛那條。
