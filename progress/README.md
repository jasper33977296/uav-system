...
> **本檔停更於 2026-08-04，此後專案已歷經多輪大改（多機、混機、驅動層抽象、
> 影像、遷台），下方內容與現況嚴重不符——它現在是有害地圖，照著走會迷路。**
>
> **現況請看**：
> - **`issues/README.md` 索引**——每個工作項的權威狀態（open／in-progress／
>   deferred／closed），PM 維護對帳。
> - **`git log` 的 commit 訊息**——逐批交付的實際內容與驗證方式
>   （本專案 commit body 寫得完整，就是開發紀錄本身）。
> - 大案的進度節寫在各自 issue 檔內（如 026 的 B0–B3）。
>
> 保留本檔僅因 [log/](log/) 的早期開發紀錄仍有史料價值。
