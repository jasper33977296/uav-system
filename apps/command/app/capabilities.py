...
ENDPOINT_CAP = {
    ...
    "emergency_land": "land",
    # 參數讀寫（2026-09-07）。**吃 `arm` 的能力鍵**：能力四態要防的是
    # 「這個機型的模式編號我方沒驗過，送過去可能切到別的東西」，而 PARAM_SET
    # 是標準 MAVLink、沒有廠牌歧義——它沒有自己的能力語意。掛 `arm` 是取
    # 「這台機我方確實指揮得動」這一件事：連解鎖都不敢放行的機型，
    # 更不該讓它改參數
    "param_get": "arm", "param_set": "arm",
}
