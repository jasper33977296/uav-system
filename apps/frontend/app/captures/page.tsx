...
interface Coverage {
  ...
  blackouts: { from: number; to: number; seconds: number; reason: string;
    recovered_by: string | null; covered_onboard: boolean | null }[];
  onboard: { name: string; bytes: number;
  ...
  return (
    <div className="page-pad">
      <div className="drone-head">
        <span className="name">錄製與回傳</span>
      ...
      {/* ① 兩層覆蓋帶 */}
      {cov && covSession && <CoverageCard cov={cov} s={covSession} />}
...
/** 兩層覆蓋帶：**這是兩層並存的全部理由，所以它要能被檢驗**。 */
function CoverageCard({ cov, s }: { cov: Coverage; s: Session }) {
    ...
    <div className="card cap-cov">
      <h3>
        ...
        <span className="cap-hint">
          {clock(s.started_at)} – {cov.ended ? hhmm(cov.to) : "進行中"} · {dur(span)}
        </span>
      </h3>
