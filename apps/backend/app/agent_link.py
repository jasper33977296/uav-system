...
class AgentLink:
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            # 040 A3：正在換號並重開飛控。**這一格不能省**——換號時機會用舊號碼
            # 消失、用新號碼回來，沒有它，那個消失看起來就是一次失聯，
            # 而 039 選項 C 會據此動作：**我們會為了修好身分而讓飛機以為地面站
            ...
            # **橋的飛控那一端**（050 需求 3、059 B）：代理與飛控之間的序列埠。
            # 收不到 MAVLink 時畫面只知道「斷了」；有這一格才說得出斷在哪一段。
            # 逐鍵挑的鏡像——少了這行，代理送得再好畫面也永遠看不到（同上）
            "fc_link": p.get("fc_link"),
