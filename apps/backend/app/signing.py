...
## ⛔ 2026-09-02：整條簽章線**裁定不做**

使用者裁定「簽章相關的東西跳過不做」。本模組**留著但不會前進**——
它做的指紋自檢在所有機上都會回 `not_enabled`，**沒有在保護任何東西**。

**留著而不是刪掉的理由**：它是無害的（不改變任何飛行行為）、有測試、
而且日後重啟這條線時，最花時間的不是這段程式而是**設計裡查證過的那些事實**
（要保護哪一段、那顆沒有電池的 RTC 會怎麼弄壞時戳）。
重啟觸發條件寫在 `doc/mavlink-signing-design.md` 開頭。

**但不要把它讀成「我們有簽章」**——下面這段說明的每一句都仍然成立，
尤其是「不要讓它被當成一道其實不存在的防線」。要刪掉它也完全合理，
說一聲就好。
