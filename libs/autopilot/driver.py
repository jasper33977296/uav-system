...
class AutopilotDriver(Protocol):
    ...
    def battery_low_params(self) -> dict[str, str]:
        """低電量門檻的參數名：`{"low_volt": …, "low_pct": …}`（有哪個給哪個）。

        **與 `gcs_failsafe_params` 同一條紀律：只回名字，不回值也不回數字。**
        門檻是逐台機的事實（幾 cell、什麼電池），要向飛控讀。

        為什麼要有這個：機上代理在失聯接管期間要自己判斷「該不該回家」，
        而**它不該自己編一個電壓門檻**——編出來的數字與飛控自己的設定不一致
        時，兩層會在不同的時間點動作，而沒有人說得出為什麼。向飛控要它自己
        的門檻，改一個地方兩層一起對。

        不知道的廠牌回空表——沒有意見時不要製造意見。
        """
        return {}
