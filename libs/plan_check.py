...
def check_terrain(nav: list[dict], home: dict, dem=None,
    ...
    **不是只檢查航點，是檢查整條線**：兩個航點之間隔著一個土坡，
    ...
    if below:
        ...
        out["warnings"].append(
            "地形資料是 SRTM（水平約 30 m）——**被格子抹平的表面**，"
            "樹冠與屋頂混在裡面，但畫不出任何一棟樓。它能防「整片地高了幾公尺」，"
            "防不了「前面有個 1 m 土堆」。真的要貼地飛需要測距儀")
        notes.append(out["warnings"][-1])
    return out
...
def _fence_norm(fence: dict | None) -> dict | None:
    ...
    def circ(c):
        return {"lat": round(float(c["lat"]), 7), "lon": round(float(c["lon"]), 7),
                "radius": round(float(c["radius"]), 2)}
